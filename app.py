...
def _token_to_hira(t) -> str:
    surf = t.surface()
    if t.part_of_speech()[0] in ["記号", "補助記号", "未知語"]:
        return surf
...
def to_hiragana(text: str, spaced: bool = False) -> str:
    ...
    return " ".join(chunks) if spaced else "".join(chunks)

# --- ローマ字（ひらがな結合後のまとまりで） ---
def to_romaji(text: str, spaced: bool = False) -> str:
    hira_spaced = to_hiragana(text, spaced=True)  # 「しました」などは1まとまり
...
def webhook():
    ...
    for ev in events:
        ...
        if cmd == "hira":
            ...
            reply_message(ev["replyToken"], "Đã bật hiển thị Hiragana." if val else "Đã tắt hiển thị Hiragana.")
            continue
        if cmd == "romaji":
            set_state(chat_id, show_romaji=val)
            reply_message(ev["replyToken"], "Đã bật hiển thị Romaji." if val else "Đã tắt hiển thị Romaji.")
            continue
        if cmd == "status":
            s = get_state(chat_id)
            reply_message(ev["replyToken"], f"Cài đặt hiện tại\n- Hiragana: {'ON' if s['show_hira'] else 'OFF'}\n- Romaji: {'ON' if s['show_romaji'] else 'OFF'}")
            continue
